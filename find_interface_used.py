...
class UsageChecker(ast.NodeVisitor):
    def __init__(
        self, target_name, is_function, aliases, show_lines=False, verbose=False
    ):
...
def is_interface_used(code, target_name, is_function, show_lines=False, verbose=False):
    try:
        ...
        tracker.visit(tree)
        target_aliases = [
            alias for alias, full in tracker.imports.items() if full == target_name
        ]
        target_aliases.append(target_name.split(".")[-1])
        checker = UsageChecker(
            target_name, is_function, target_aliases, show_lines, verbose
        )
        checker.visit(tree)
        return {"used": checker.used, "locations": checker.usage_locations}
    except SyntaxError:
        return {"used": False, "locations": []}


def find_interface_used(
    interface_name: str,
    target_lib: str,
    codebase_path: str,
    show_lines=False,
        ...
        for file in files:
            if file.endswith(".py"):
                ...
                with open(file_path, "r") as f:
                    code = f.read()
                    usage = is_interface_used(
                        code, target_name, is_function, show_lines, verbose
                    )
                    if usage["used"]:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find where an interface is used in a codebase"
    )
    ...
    parser.add_argument(
        "--show-lines",
        action="store_true",
        help="Show line numbers where interface is used",
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed output with code context"
    )
    args = parser.parse_args()

    results = find_interface_used(
        args.interface, args.library, args.path, args.show_lines, args.verbose
    )
